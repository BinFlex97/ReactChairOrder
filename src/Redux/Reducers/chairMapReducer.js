...
export const chairListReducer = (state = stateChairList, action) => {
  switch (action.type) {
    case "CHAIR_SELECT":
      state.chairSelected.danhSachGhe = [...state.chairSelected.danhSachGhe, action.ghe]
      state.chairSelected.tongTien += action.ghe.gia
      state.chairSelected = {...state.chairSelected}
      return { ...state };
    case "CHAIR_CONFIRM":
      state.chairSelected.danhSachGhe.map((chairSelec) => {
        state.chairList.map((chairL) => {
          let index = chairL.danhSachGhe.findIndex(chairFind => chairFind.soGhe == chairSelec.soGhe)
          if (index != -1) {
            chairL.danhSachGhe[index].daDat = true;
          }
        })
      })
      state.chairList = [...state.chairList]
      state.chairSelected.danhSachGhe = []
      state.chairSelected.tongTien = 0
      state.chairSelected = {...state.chairSelected}
      return { ...state };
    case "DELETE_CHAIR":
      state.chairSelected.tongTien -= action.ghe.gia
      if(state.chairSelected.tongTien <0) state.chairSelected.tongTien = 0
      state.chairSelected.danhSachGhe = [...state.chairSelected.danhSachGhe.filter(chairFilter => chairFilter.soGhe != action.ghe.soGhe)]
      state.chairSelected = {...state.chairSelected}
      return { ...state };
    default:
      return { ...state };
